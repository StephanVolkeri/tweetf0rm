...
import logging
import argparse, os, sys, multiprocessing, time, shutil
#from tweetf0rm.user_farm import UserFarm
    ...
    while current_user_id != -1:
        ...
        if retry and retry_cnt > 0:
            ...
            if current_user_id == -1:
                if len(
                        problem_queue) > 0:  # have issues with a few user_id, we try to add them back to the queue to retry
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--seeds',
                        help="the config file for defining seed users and depth; see twitter_crawler.json as an example",
                        required=True)
