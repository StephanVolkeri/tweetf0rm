...
class TwitterAPI(twython.Twython):
    ...
    def rate_limit_error_occured(self, resource, api):
        ...
        logger.warning('[%s] rate limit reached, sleep for %d' % (rate_limits['rate_limit_context'], wait_for))
        if wait_for < 0:
            wait_for = 60
    ...
    def fetch_user_timeline(self, user_id=None, write_to_handlers=[], cmd_handlers=[], bucket="timelines"):
        ...
        retry_cnt = MAX_RETRY_CNT
        timeline = []  # holder tweets in memory... you won't get more than 3,200 tweets per user,
